...
impl Scalar {
  ...
  pub fn invert(&self) -> CtOption<Self> {
    ...
    if result.is_some() {
        ...
        // let result_bytes: [u8; 64] = result.try_into().unwrap_or_else(|_| {
        //     panic!("Unexpected length of result vector; expected 32 bytes")
        // });
  ...
  pub const fn mul(&self, rhs: &Self) -> Self {
    ...
    // let (r4, r5) = mac(0, self.0[0], rhs.0[4], carry); // 0 + (self.0[0] * rhs.0[4]) + carry = carry
    ...
    // let (r4, carry) = mac(r4, self.0[4], rhs.0[0], 0); // r4 + (self.0[4] * rhs.0[0]) + 0 = r4
...
mod tests {
  ...
  fn test_debug() {
    ...
    // println!("In test_debug R2 = {:?}; R2_format {:?}", R2, R2_format);
    // assert_eq!(
    //   format!("{:?}", R2),
  ...
  fn test_to_bytes() {
    ...
    // println!("test t256 scalar successfully -1 = {:?}", ScalarField::from(-1));
    // assert_eq!(
    //   R2.to_bytes(),
    //   [
