...
public class MoproFlutterPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "crescentInitializeCache":
      guard let args = call.arguments as? [String: Any],
        let schemeName = args["schemeName"] as? String,
        let mainWasm = args["mainWasm"] as? FlutterStandardTypedData,
        let mainR1cs = args["mainR1cs"] as? FlutterStandardTypedData,
        let groth16Pvk = args["groth16Pvk"] as? FlutterStandardTypedData,
        let groth16Vk = args["groth16Vk"] as? FlutterStandardTypedData,
        let proverParams = args["proverParams"] as? FlutterStandardTypedData,
        let rangePk = args["rangePk"] as? FlutterStandardTypedData,
        let rangeVk = args["rangeVk"] as? FlutterStandardTypedData,
        let ioLocations = args["ioLocations"] as? String
      else {
      ...
      do {
        let assetBundle = AssetBundle(
          mainWasm: mainWasm.data,
          mainR1cs: mainR1cs.data,
          groth16Pvk: groth16Pvk.data,
          groth16Vk: groth16Vk.data,
          proverParams: proverParams.data,
          rangePk: rangePk.data,
          rangeVk: rangeVk.data,
          ioLocations: ioLocations
        )
        let cacheId = try crescentInitializeCache(schemeName: schemeName, assetBundle: assetBundle)
        result(cacheId)
      } catch {
        result(
          FlutterError(
            code: "CRESCENT_CACHE_ERROR", message: "Failed to initialize cache",
            details: error.localizedDescription))
      }
    ...
    case "crescentProve":
      guard let args = call.arguments as? [String: Any],
        ...
        let issuerPem = args["issuerPem"] as? String,
      ...
      let devicePubPem = args["devicePubPem"] as? String

      do {
        let clientState = try crescentProve(
          cacheId: cacheId,
          jwtToken: jwtToken,
          issuerPem: issuerPem,
        ...
        result(
          FlutterError(
            code: "CRESCENT_PROVE_ERROR", message: "Failed to generate client state",
            details: error.localizedDescription))
      }
    ...
    case "crescentShow":
      guard let args = call.arguments as? [String: Any],
        let cacheId = args["cacheId"] as? String,
        let clientStateB64 = args["clientStateB64"] as? String,
        let proofSpecJson = args["proofSpecJson"] as? String
      else {
        result(FlutterError(code: "ARGUMENT_ERROR", message: "Missing required arguments", details: nil))
        return
      }

      let presentationMessage = args["presentationMessage"] as? String
      let devicePrvPem = args["devicePrvPem"] as? String

      do {
        let showProof = try crescentShow(
          cacheId: cacheId,
        ...
        result(
          FlutterError(
            code: "CRESCENT_SHOW_ERROR", message: "Failed to generate presentation proof",
            details: error.localizedDescription))
      }
    ...
    case "crescentVerify":
      guard let args = call.arguments as? [String: Any],
        let cacheId = args["cacheId"] as? String,
        let showProofB64 = args["showProofB64"] as? String,
        let proofSpecJson = args["proofSpecJson"] as? String,
        let issuerPem = args["issuerPem"] as? String,
        let configJson = args["configJson"] as? String
      else {
      ...
      do {
        let verifyResult = try crescentVerify(
          cacheId: cacheId,
        ...
        result(
          FlutterError(
            code: "CRESCENT_VERIFY_ERROR", message: "Failed to verify presentation proof",
            details: error.localizedDescription))
      }

    case "crescentCleanupCache":
      guard let args = call.arguments as? [String: Any],
        let cacheId = args["cacheId"] as? String
      else {
        result(FlutterError(code: "ARGUMENT_ERROR", message: "Missing required arguments", details: nil))
      ...
      do {
        try crescentCleanupCache(cacheId: cacheId)
        result(nil)
      } catch {
        result(
          FlutterError(
            code: "CRESCENT_CACHE_ERROR", message: "Failed to cleanup cache",
            details: error.localizedDescription))
      }
